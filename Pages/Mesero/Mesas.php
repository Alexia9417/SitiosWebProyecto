...
$idMesero = $_SESSION['IdUsuario']; // Asegúrate de que el ID del mesero esté almacenado en la sesión

// Consulta para obtener las mesas asociadas al mesero y verificar si tienen órdenes
$query = "
    SELECT m.Numero, IF(COUNT(o.IdOrden) > 0, TRUE, FALSE) as TieneOrden
    FROM mesa m
    LEFT JOIN orden o ON m.Numero = o.IdMesa
    WHERE m.IdMesero = ?
    GROUP BY m.Numero
";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $idMesero); // "i" indica que el parámetro es un entero
$stmt->execute();
...
// Consulta para obtener las mesas asociadas al mesero y verificar los estados de los checkboxes
$querycheck = "
    SELECT
        m.Numero,
        EXISTS (
            SELECT 1 FROM orden o
            WHERE o.IdMesa = m.IdMesa
        ) AS TieneOrden,
        IFNULL((
            SELECT IF(COUNT(*) = SUM(CASE WHEN do.IdEstadoPlatillo = (SELECT idEstadoPlato FROM estado_plato WHERE Nombre = 'Listo') THEN 1 ELSE 0 END), TRUE, FALSE)
            FROM detalle_orden do
            JOIN orden o ON do.IdOrden = o.IdOrden
            WHERE o.IdMesa = m.IdMesa
        ), FALSE) AS ComidaServida,
        EXISTS (
            SELECT 1 FROM pagos p
            WHERE p.IdMesa = m.Numero
        ) AS CuentaPagada
    FROM
        mesa m
    WHERE
...
<head>
  ...
  <title>Panel del Mesero - Mesas</title>
...
<style>
  ...
  .contenedor-principal {
    ...
    max-width: 1000px;
    /* suficiente para varias tarjetas */
    margin: auto;
  }
  ...
  /* Contenedor grid que acomoda las tarjetas */
  .grid-mesas {
    display: grid;
...
<body>
        ...
        <div class="grid-mesas">
          <?php foreach ($mesas as $mesa): ?>
            <div class="tarjeta-mesa">
              ...
              <div class="estado">
                <input type="checkbox" <?php echo $mesa['TieneOrden'] ? 'checked' : ''; ?> disabled> Orden recibida <br>
                <input type="checkbox" <?php echo $mesa['ComidaServida'] ? 'checked' : ''; ?> disabled> Comida Servida <br>
                <input type="checkbox" <?php echo $mesa['CuentaPagada'] ? 'checked' : ''; ?> disabled> Cuenta Pagada
              </div>
              <div class="interaccion">
                <i class="fas fa-clock"></i> Última interacción: hace 15 min
              </div>
              <button class="accion">Esperar Mesa</button>
            </div>
          <?php endforeach; ?>
  ...
  <!-- Script para manejar el estado de los botones -->
  <script>
    document.querySelectorAll('.tarjeta-mesa').forEach(tarjeta => {
      const checks = tarjeta.querySelectorAll('input[type="checkbox"]');
      const boton = tarjeta.querySelector('.accion');
      if (checks[0].checked && !checks[1].checked) {
        boton.textContent = 'Mandar Orden';
      } else if (checks[1].checked && !checks[2].checked) {
        boton.textContent = 'Esperar Mesa';
      } else if (checks[2].checked) {
        boton.textContent = 'Preparar Cuenta';
      } else {
        boton.textContent = 'En Espera';
      }
    });
  </script>
  <!-- Script para manejar el estado de los botones -->
