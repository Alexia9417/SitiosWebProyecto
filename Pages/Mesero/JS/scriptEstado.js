...
function disminuirEnergiaYEficiencia() {
    ...
    estres = Math.min(100, estres + 1); // Aumenta 1 cada 5 segundos
    actualizarInterfaz();
    guardarEstado();
}
...
function descanso() {
    //console.log("Antes descanso:", { estres, eficiencia });
    energia = Math.min(100, energia + 3);
    estres = Math.max(0, estres - 3);
    eficiencia = Math.min(100, eficiencia + 2);

    //console.log("Después descanso:", { estres, eficiencia });
    actualizarInterfaz();
    guardarEstado();
}
