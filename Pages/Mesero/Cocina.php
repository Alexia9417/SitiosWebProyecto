...
<body>
  ...
  <script>
    ...
    <!--Script para manejar el boton de preguntar-->
    <script>
        document.querySelectorAll('.boton-preguntar').forEach(boton => {
            boton.addEventListener('click', function() {
                const ordenDiv = this.closest('.orden');
                const idOrden = ordenDiv.querySelector('.orden-cabecera strong').textContent.replace('Orden Mesa #', '');
                const mensaje = this.textContent;
                ...
                fetch('enviar_mensaje.php', {
                        ...
                        body: `id_orden=${idOrden}&id_mesero=<?php echo $idMesero; ?>&mensaje=${mensaje}`
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            alert('Mensaje enviado a cocina');
                        } else {
