...
$sqlPropinas = "SELECT MontoPropina FROM accioncliente WHERE IdMesero = ? AND TipoAccion = 'Propina'";
...
// Consulta para obtener las tareas pendientes del mesero
$sqlTareasPendientes = "SELECT id, descripTarea, IdMesa, TiempoEstimado FROM tareas WHERE idMesero = ? AND Estado = 'Pendiente'";
$stmtTareasPendientes = $conn->prepare($sqlTareasPendientes);
$stmtTareasPendientes->bind_param("i", $idMesero);
$stmtTareasPendientes->execute();
$resultTareasPendientes = $stmtTareasPendientes->get_result();
...
$stmtTareasCompletadas = $conn->prepare($sqlTareasCompletadas);
$stmtTareasCompletadas->bind_param("i", $idMesero);
$stmtTareasCompletadas->execute();
$resultTareasCompletadas = $stmtTareasCompletadas->get_result();

$tareasCompletadas = 0;
if ($resultTareasCompletadas->num_rows > 0) {
  $rowTareasCompletadas = $resultTareasCompletadas->fetch_assoc();
  $tareasCompletadas = $rowTareasCompletadas['TareasCompletadas'];
}
...
<head>
  ...
  <title>Panel del Mesero - Tareas</title>
...
<style>
  ...
  /* CABECERA DE TAREA (Título + tiempo) */
  .tarea-cabecera .tiempo {
    font-size: 0.9rem;
    color: #666;
  }

  /* DETALLES (Prioridad + botón) */
  .tarea-detalle {
    display: flex;
    justify-content: space-between;
    align-items: center;
...
<body>
    ...
    <div class="contenedor-principal">
      ...
      <div class="tareas">
        <div class="Lista-Tareas">
          <?php
          if ($resultTareasPendientes->num_rows > 0) {
            while ($rowTarea = $resultTareasPendientes->fetch_assoc()) {
              echo '<div class="tarea">';
              echo '<div class="tarea-cabecera">';
              echo '<span><strong>' . htmlspecialchars($rowTarea['descripTarea']) . '</strong></span>';
              echo '<br>';
              echo '<span class="tiempo">' . htmlspecialchars($rowTarea['TiempoEstimado']) . ' min</span>';
              echo '</div>';
              echo '<div class="tarea-detalle">';
              echo '<span>Prioridad: <strong class="prioridad media">Media</strong></span>';
              echo '<button class="boton-completar" data-tarea-id="' . $rowTarea['id'] . '">Completar</button>';
              echo '</div>';
              echo '</div>';
            }
          } else {
            echo '<p style="color: white;">No hay tareas pendientes.</p>';
          }
          ?>
        </div>
      </div>
  ...
  <!--Resetear estados al cerrar sesion-->
  <script>
