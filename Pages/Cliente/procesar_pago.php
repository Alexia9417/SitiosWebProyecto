...
try {
    ...
    $metodoPago = "Tarjeta de crédito"; // Método de pago fijo
    $stmtPago = $conn->prepare("INSERT INTO pagos (IdMesa, Monto, FechaPago, MetodoPago) VALUES (?, ?, NOW(), ?)");
    $stmtPago->bind_param("ids", $idMesa, $totalConDeuda, $metodoPago);
    if (!$stmtPago->execute()) throw new Exception("Error al registrar el pago.");
    $stmtPago->close();
