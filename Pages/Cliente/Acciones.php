    ...
    <script>
        ...
        function enviarCalificacion() {
            ...
            fetch('GuardarCalificacion.php', {
                ...
                .then(res => res.text())
                .then(data => {
                    mensaje.style.color = data.toLowerCase().includes('éxito') ? 'white' : 'red';
                    mensaje.textContent = data;
        ...
        document.querySelectorAll('.mini-cuadroIzquierda')[5].addEventListener('click', () => {
            ...
            fetch('VerificarSalida.php', {
                ...
                .then(data => {
                    ...
                    if (data === 'tiene_orden') {
                        texto.textContent = '¿Estás seguro de salir sin pagar? Lo que no ha pagado se le cobrará en la siguiente visita.';
                    } else {
        ...
        document.querySelectorAll('.mini-cuadroIzquierda')[1].addEventListener('click', () => {
            ...
            fetch('GuardarNotificacion.php', {
                ...
                .then(res => {
                    if (!res.ok) {
                        throw new Error('La respuesta del servidor no es correcta.');
                    }
                    return res.text();
                })
                .then(data => {
